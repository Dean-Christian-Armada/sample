...
def _AddVpnHop(next_hop_group):
  """Attach arguments for beta next-hops to the a parser group."""
...
def _Args(parser, include_vpn):
  """Add arguments for route creation, optionally including vpn support."""
...
def _CreateRequests(cmd, args):
  ...
  if getattr(args, 'next_hop_vpn_tunnel', None):
...
class Create(base_classes.BaseAsyncCreator):
  ...
  @staticmethod
  def Args(parser):
    _Args(parser, include_vpn=False)
...
class CreateBeta(base_classes.BaseAsyncCreator):
  """Create a new route in Beta."""

  @staticmethod
  def Args(parser):
    _Args(parser, include_vpn=True)
  ...
  return {
      ...
      'DESCRIPTION': """\
          *{command}* is used to create routes. A route is a rule that
          ...
          ``--next-hop-instance'',{vpn} or ``--next-hop-gateway'' of the
          winning route. Packets that do not match any route in the
          sending virtual machine routing table will be dropped.

          Exactly one of ``--next-hop-address'', ``--next-hop-gateway'',{vpn}
          or ``--next-hop-instance'' must be provided with this
          command.
          """.format(vpn=vpn_hop, command='{command}'),
      }

Create.detailed_help = _MakeDetailedHelp(include_vpn=False)
CreateBeta.detailed_help = _MakeDetailedHelp(include_vpn=True)
