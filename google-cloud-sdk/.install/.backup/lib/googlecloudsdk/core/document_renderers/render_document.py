...
class _MarkdownConverter(object):
  ...
  Attributes:
    ...
    _edit: True if NOTES or _root_old => _root_new edits are required.
    ...
    _root_new: If _edit then _root_new replaces _root_old in rendered output.
    _root_old: If _edit then _root_new replaces _root_old in rendered output.
  """
  _EMPHASIS = {'*': renderer.BOLD, '_': renderer.ITALIC, '`': renderer.CODE}

  def __init__(self, style_renderer, fin=sys.stdin, notes=None, root=None):
    ...
    Args:
      ...
      root: root="gcloud foo" renders "gcloud foo ..." as "foo ...".
    """
    ...
    self._root_old = None
    self._root_new = None
    # TODO(user): The root processing will be ripped out in another cl that
    # unfies document generation for all SDK components. Its only used in
    # compute right now.
    if root:
      i = root.rfind(' ')
      if i > 0:
        self._root_old = root
        self._root_new = root[i + 1:]
    self._edit = self._notes or self._root_old
  ...
  def _ReadLine(self):
    """Reads and preprocesses the next markdown line fron self._fin.
    ...
    if self._root_old:
      line = line.replace(self._root_old, self._root_new)
    return line
...
def RenderDocument(style='text', fin=None, out=None, width=80, notes=None,
  ...
  style_renderer = _STYLES[style](out=out or sys.stdout, title=title,
                                  width=width)
  _MarkdownConverter(style_renderer, fin=fin or sys.stdin, notes=notes,
                     root=root).Run()
...
def main(argv):
  ...
  parser.add_argument(
      '--root',
      metavar='COMMAND',
      help='The document root command. --root="gcloud foo" renders'
      '"gcloud foo ..." as "foo ...".')
  ...
  RenderDocument(args.style, notes=args.notes, root=args.root, title=args.title)
